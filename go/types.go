// Package colornamefinder provides a Go client for the Color Name Finder API.
//
// For more information, visit: https://apiverve.com/marketplace/colornamefinder?utm_source=go&utm_medium=readme
package colornamefinder
...
// Request contains the parameters for the Color Name Finder API.
//
// Parameters:
//   - hex (required): string - Hex color value (with or without # prefix) [format: hexColor]
//   - closest: integer - Number of closest color matches to return [min: 1, max: 20]
type Request struct {
	Hex string `json:"hex"` // Required
	Closest int `json:"closest,omitempty"` // Optional
}
...
func (r *Request) Validate() error {
	rules := map[string]ValidationRule{
		"hex": {Type: "string", Required: true, Format: "hexColor"},
		"closest": {Type: "integer", Required: false, Min: float64Ptr(1), Max: float64Ptr(20)},
	}
...
// ResponseData contains the data returned by the Color Name Finder API.
type ResponseData struct {
	InputHex string `json:"input_hex"`
	InputRgb InputRgbData `json:"input_rgb"`
	ExactMatch bool `json:"exact_match"`
	ClosestColor ClosestColorData `json:"closest_color"`
	ClosestMatches []ClosestMatchesItem `json:"closest_matches"`
	TotalNamedColors int `json:"total_named_colors"`
}

// InputRgbData represents the input_rgb object.
type InputRgbData struct {
	R int `json:"r"`
	G int `json:"g"`
	B int `json:"b"`
}

// ClosestColorData represents the closest_color object.
type ClosestColorData struct {
...
// ClosestMatchesItem represents an item in the ClosestMatches array.
type ClosestMatchesItem struct {
	Name string `json:"name"`
	Hex string `json:"hex"`
	Distance float64 `json:"distance"`
	Similarity float64 `json:"similarity"`
